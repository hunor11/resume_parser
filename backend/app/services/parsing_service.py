    ...
    def parse_directory(self) -> dict[str, list[Document]]:
        ...
        for file_path in self.data_dir.glob("*"):
            ...
            try:
                ...
                chunks = self._split_documents(docs)
                # normalize source metadata on each chunk
                for ch in chunks:
                    ch.metadata.setdefault("source", str(file_path))
                results[file_path.name] = chunks
            except Exception:
                # propagate an empty list for failed files; log upstream if needed
                results[file_path.name] = []
                # You can also raise or log here depending on your policy
        return results
